...
var macPattern = regexp.MustCompile(`^([A-Fa-f0-9]{2}-){5}[A-Fa-f0-9]{2}$|^([A-Fa-f0-9]{2}:){5}[A-Fa-f0-9]{2}$`)
...
func (m Mac) DefaultMessage() string {
	return MessageTmpls["Mac"]
}

// GetKey return the i.Key
func (m Mac) GetKey() string {
	return m.Key
}

// GetLimitValue return the limit value
func (m Mac) GetLimitValue() interface{} {
	return nil
}
...
// 中文验证(只匹配中文汉字)
var chinesePattern = regexp.MustCompile("[\u4e00-\u9fa5]")
